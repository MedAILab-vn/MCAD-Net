...
class Config:
    def __init__(self, **kwargs):
        ...
        # 1. EXPERIMENT FLAGS
        # ==========================================
        # Các models: MCAT_Model (multi scale, cross modal), MCAT_Model_cs(cross scale, cross modal)
        self.model_type = 'MCAT_Model_SynScale2'
        # Các module hỗ trợ: 'CBAM', 'ECA', 'GAM', 'TripletAM', 'EMA', 'LSKNet', 'None'
        self.attention_type = 'None'
        self.fused_block = 'None'
        self.custom_ext = ''
        self.save_model = False

        # Chế độ chạy: 'single' (chia 8:2) hoặc 'kfold' (Cross Validation)
        ...
        # ==========================================
        # 2. CẤU HÌNH LOSS & BAYESIAN (UNCERTAINTY)
        # ==========================================
        # Chế độ Loss lúc Train: 'CE' (bình thường) hoặc 'UAL' (Uncertainty-Aware Loss)
        self.loss_type = 'CE'
        self.mc_passes_train = 20  # Số lần chạy vòng lặp MC-Dropout lúc train cho UAL

        # Chế độ Inference lúc Test:
        self.bayesian_test = True  # True: Bật Bayesian (MC-Dropout) lúc test. False: Test 1 lần tĩnh.
        self.mc_passes_test = 50  # Số lần lấy mẫu lúc test (đã đổi tên từ mc_passes cho đồng bộ)
        self.entropy_threshold = 0.5  # Ngưỡng phân loại High Uncertainty Samples (HUS)

        # ==========================================
        # 3. CẤU HÌNH HỆ THỐNG (SYSTEM & HARDWARE)
        # ==========================================
        self.gpus = '0'
        self.embed_dim = 256
        self.num_workers = 0
        ...
        # ==========================================
        # 4. ĐƯỜNG DẪN DỮ LIỆU (DATA PATHS)
        # ==========================================
        ...
        # ==========================================
        # 5. THAM SỐ MÔ HÌNH (MODEL PARAMS)
        # ==========================================
        self.input_size = 448
        self.num_classes = 4
        self.embed_dim = 256
        ...
        # ==========================================
        # 6. THAM SỐ HUẤN LUYỆN (TRAINING PARAMS)
        # ==========================================
        self.epochs = 60
        self.batch_size = 16
        ...
        self.use_supcon = False  # Công tắc bật/tắt (True: Dùng MCAT_Model_SupCon, False: Dùng cũ)
        self.beta_supcon = 0.5  # Trọng số beta cho SupCon Loss (Nên thử nghiệm ở mức 0.1 -> 0.5)
        self.supcon_temperature = 0.07  # Nhiệt độ chuẩn của hàm InfoNCE/SupCon

        self.optimize_metric = 'F1-score' #'AUC'
        self.early_stopping_patience = 10
        self.unfreeze_all = False
        self.freeze_scale1 = False

        self.lr_backbone = 1e-3  # LR siêu nhỏ cho pre-trained weights
        self.lr_head = 1e-3  # LR tiêu chuẩn cho các lớp Attention/Classifier mới
        self.dropout_head = 0.2  # Dropout thấp để chống Overfitting bình thường
        self.dropout_mc = 0.5  # Dropout cao để tạo độ bất định cho Monte Carlo / UAL

        self.cfp_attention_type = None
        self.oct_attention_type = None

        # ==========================================
        # 8. GHI ĐÈ THAM SỐ TỪ EXTERNAL CALL
        # ==========================================
        # Đảm bảo lệnh này nằm cuối cùng để cập nhật các giá trị truyền từ main()
        for key, value in kwargs.items():
            setattr(self, key, value)

        # Cập nhật tên thư mục gốc để chứa cả thông tin loại Loss đang dùng
        ...
        if self.input_size == 224:
            ...
            if self.model_type == 'MCAT_Model_GhostNet':
            ...
            if self.model_type == 'MCAT_Model_SynScale2_b1':
        ...
        if self.dataset_name == 'mmc-amd':
            ...
            # Sửa lại đường dẫn CSV của bạn (nhớ file CSV phải có cột 'label' chứa số 0, 1, 2, 3)
            #self.csv_path = r'D:\Projects\Personal\Master\OCT\SourceCode\FairVision\dataset\HarvardFairVision\AMD\your_file.csv'

            # Trỏ đến thư mục chứa ảnh SLO và OCT tương ứng
    ...
    def get_output_dir(self, fold=None):
        """
        Hàm tự động sinh tên thư mục lưu trữ bài bản, tránh ghi đè.
        """
        base_dir = './output_updateJune2026'

        # Khung tên model dựa trên cờ attention và loss (ví dụ: MCAT_Res18_EffB0_LSKNet_UAL)
        model_name = f"MCAT_Res18_EffB0_{self.attention_type}_{self.loss_type}"

        # Chuỗi tham số hyperparams
        param_str = f"bs{self.batch_size}_lr{self.learning_rate}_wd{self.weight_decay}_rs{self.random_seed}"

        # Chuẩn hóa tên theo chế độ chạy
        if self.run_mode == 'kfold':
            mode_str = f"KFold{self.k_folds}"
            if fold is not None:
                mode_str += f"_F{fold}"  # Ví dụ: KFold5_F1
        else:
            mode_str = "SingleRun"

        # Ghép lại thành folder hoàn chỉnh
