...
def main(current_seed, dname, alpha=0.01, branch_attention_type=None, dropout=0.5, loss_type="CE", batch_size=16,
    ...
    if config.run_mode == 'train_val_test' or config.dataset_name.startswith('HarvardFairVision'):
        ...
        # KIỂM TRA CỜ seperate_test TỪ config
        seperate_test = getattr(config, 'seperate_test', False)
        ...
        if seperate_test:
            ...
            df_train_all = pd.read_csv(config.csv_path, sep=None, engine='python', encoding='utf-8-sig')
            df_test = None
        ...
        if config.run_mode == 'single':
            ...
            test_info = f" | Test: {len(df_test)}" if df_test is not None else ""
            print(f"Total images -> Train: {len(train_df)} | Val: {len(val_df)}{test_info}")
            ...
            for fold, (train_idx, val_idx) in enumerate(fold_splits, 1):
                ...
                test_info = f" | Test: {len(df_test)}" if df_test is not None else ""
                print(f"Train: {len(train_df)} | Val: {len(val_df)}{test_info}")
